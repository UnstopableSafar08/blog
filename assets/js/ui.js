...
const UI = (() => {
    ...
    // ── Edit User Modal ───────────────────────────────────────────
    function showEditUserModal(id, currentUsername, currentRole) {
        return new Promise(resolve => {
            const overlay = document.createElement('div');
            overlay.className = 'confirm-overlay';
            overlay.innerHTML = `
                <div class="confirm-dialog" style="max-width: 440px;">
                    <h3>Edit User</h3>
                    <p style="margin-bottom: 20px; color: var(--text-secondary);">Update username and role for this user.</p>
                    <div class="form-group" style="margin-bottom: 16px;">
                        <label class="form-label" for="edit-modal-username">Username</label>
                        <input class="form-input" type="text" id="edit-modal-username" value="${escapeHtml(currentUsername)}" required>
                    </div>
                    <div class="form-group" style="margin-bottom: 24px;">
                        <label class="form-label" for="edit-modal-role">Role</label>
                        <select class="form-input" id="edit-modal-role" style="appearance: auto;">
            ...
            setTimeout(() => overlay.querySelector('#edit-modal-username').focus(), 100);

            overlay.querySelector('#edit-modal-cancel').addEventListener('click', () => {
                overlay.remove();
                resolve(null);
            });

            overlay.querySelector('#edit-modal-save').addEventListener('click', () => {
                const newUsername = overlay.querySelector('#edit-modal-username').value.trim();
                const newRole = overlay.querySelector('#edit-modal-role').value;
                overlay.remove();
                if (!newUsername) {
                    resolve(null);
                } else {
                    resolve({ username: newUsername, role: newRole });
                }
            });
            ...
            overlay.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    overlay.querySelector('#edit-modal-save').click();
        ...
        return new Promise(resolve => {
            ...
            overlay.innerHTML = `
                <div class="confirm-dialog" style="max-width: 440px;">
                    ...
                    <p style="margin-bottom: 20px; color: var(--text-secondary);">Set a new password for <strong>${escapeHtml(username)}</strong></p>
                    <div class="form-group" style="margin-bottom: 16px;">
                        <label class="form-label" for="pwd-modal-new">New Password</label>
                        <input class="form-input" type="password" id="pwd-modal-new" placeholder="Minimum 6 characters" required>
                    </div>
                    <div class="form-group" style="margin-bottom: 24px;">
                        <label class="form-label" for="pwd-modal-confirm">Confirm Password</label>
                        <input class="form-input" type="password" id="pwd-modal-confirm" placeholder="Re-enter password" required>
                    </div>
                    <div class="confirm-dialog-actions">
                        <button class="btn btn-secondary" id="pwd-modal-cancel">Cancel</button>
                        <button class="btn btn-primary" id="pwd-modal-save">Update Password</button>
                    </div>
                </div>
            `;
            document.body.appendChild(overlay);

            setTimeout(() => overlay.querySelector('#pwd-modal-new').focus(), 100);

            overlay.querySelector('#pwd-modal-cancel').addEventListener('click', () => {
                overlay.remove();
                resolve(null);
            });

            overlay.querySelector('#pwd-modal-save').addEventListener('click', () => {
                const newPwd = overlay.querySelector('#pwd-modal-new').value;
                const confirmPwd = overlay.querySelector('#pwd-modal-confirm').value;
            ...
            overlay.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    overlay.querySelector('#pwd-modal-save').click();
                } else if (e.key === 'Escape') {
                    overlay.remove();
                    resolve(null);
                }
            ...
            <tr>
                ...
                <td>
                    <div class="table-actions" style="justify-content: flex-end;">
                        ...
                        <button class="btn btn-secondary btn-sm" onclick="App.updateUserPassword('${escapeHtml(adm.username)}')">
